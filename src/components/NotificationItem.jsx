...
const NotificationItem = ({ title, highlight, time, priority, image }) => {
  const priorityColor =
    priority === "High"
      ...
      ? "text-[#D95000]" // In the image, 'High' is also red
      : "text-[#D95000]/80"; // Defaulting moderate to a lighter red based on the image's theme
      ...
      {isOpen && (
        <>
          ...
          >
            ...
            <div className="bg-[#FFFFFF] px-6 py-5 flex justify-between items-start rounded-t-2xl">
              <div>
              ...
              <button 
                onClick={onClose}
                className="text-[#D95000] hover:text-gray-900 transition-colors mt-1 mr-1"
              >
                <BsReplyFill size={24} />
              </button>
            </div>
